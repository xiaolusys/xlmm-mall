...
export class ShopBag extends Component {
  ...
  onBuyNowClick = (e) => {
    ...
    window.location.href = 'http://m.xiaolumeimei.com/pages/queren-dd.html?cart_ids=' + encodeURIComponent(cartIds.join(','));
  }
              ...
              {_.isEmpty(shopBag.data) ? null : shopBag.data.map((item) => {
                return (
                  <li key={item.id} className="row no-margin bottom-border">
                    <img className="col-xs-4" src={item.pic_path + constants.image.square} />
